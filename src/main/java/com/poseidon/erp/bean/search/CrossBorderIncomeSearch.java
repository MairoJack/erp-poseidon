...
@Getter
@Setter
public class CrossBorderIncomeSearch implements BaseSearch<CrossBorderIncome> {

    @ApiParam(value = "返款项目", name = "name")
    private String name;
    ...
    @Override
    public Wrapper<CrossBorderIncome> query() {
        return Wrappers
                .<CrossBorderIncome>lambdaQuery()
                .like(StrUtil.isNotBlank(name), CrossBorderIncome::getName,name)
                .apply(StrUtil.isAllNotEmpty(startDate, endDate), dateRange("item_time", startDate, endDate))
                .orderByDesc(CrossBorderIncome::getId)
        ;
    }
}
